...
*Figure 1. Syntax of $\lambda_{\mathrm{BDL}}$. Variables are de Bruijn indices in the development; the paper writes names. $\mathit{pf}$ in $\mathsf{eq}_\tau^{\mathit{pf}}$ is a proof that $\tau$ is a data type.*
...
Terms are those of the λ-calculus plus five design-specific forms. A constant name $\delta$ refers to a relationship by the identity of its declaration; nothing about the declaration's interface or realization is in the syntax, which is what lets a term refer to a relationship that has no realization yet. $\mathsf{rep}\;e$ observes the representation of a concept value and $\mathsf{mk}_C\;e$ constructs one — the elimination and the introduction of the abstract type $C$; $\mathsf{mk}_C\,v$ is the value form. $\mathsf{delay}\;i\;e$ is the value of $e$ at the previous activation of the current domain, $i$ before any; $\mathsf{sync}_{\kappa}\;i\;e$ is the value of $e$ in domain $\kappa$ at $\kappa$'s last activation strictly before now, $i$ if none. $\mathsf{fold}\;f\;z\;l$ is the list recursor, $\mathsf{fold}\;f\;z\;[x_1,\dots,x_n] = f\;x_1\;(\cdots(f\;x_n\;z))$. Registered operators $p$ are first-order constants with types; they never apply a closure.
...
- A **declaration** is a triple
  $$\mathsf{DesignDecl} = \langle\, \mathit{id} : \mathsf{DeclId},\ \mathit{interface} : \langle \mathit{expectedType} : \mathsf{Ty},\ \mathit{commitments} : \mathsf{PropertyId}^{*}\rangle,\ \mathit{realization} : \mathsf{Option}\;\mathsf{Expr} \,\rangle .$$
  A **design** is a global environment $\Delta$ of declarations, a partial map from constant names. Two projections of it are the only views a judgment may take: the *type view* $\Delta \ni \delta : \tau$ (the constant $\delta$ is declared at $\tau$), which is all that typing sees, and the *realization view* $\Delta \ni \delta := b$ (its definiens is $b$), which is all that evaluation sees; $\delta \notin \mathrm{def}(\Delta)$ says the constant has no definiens. An **unrealized** declaration is one without a definiens; nothing else distinguishes it. - A **concept signature** $\Theta$ binds each concept, write-once, to a representation, $\Theta(C) = R$. It is well formed, $\Theta.\mathsf{WF}$, when every bound representation is concept-free and data: $\Theta(C) = R \Rightarrow R.\mathsf{SemFree} \wedge R.\mathsf{Data}$.
- A **grant** $G$ is a set of concepts a term may construct. $\varnothing$ permits nothing; $\mathrm{grant}\;\tau$ permits the concepts in result position of $\tau$, $\mathsf{grant}(C) = [C]$, $\mathsf{grant}(\tau \to \sigma) = \mathsf{grant}(\sigma)$, $\mathsf{grant}(\_) = [\,]$.
...
Typing sees $\Theta$, the type view of $\Delta$ and $G$. Evaluation sees the realization view of $\Delta$, $I$ and (in several domains) $S$. The domain judgment sees $\mathrm{K}$. Outputs see $\Omega$, $\mathrm{K}$, $\Delta^{\mathrm{ty}}$ and $\beta$. Commitments and evidence are seen by the satisfaction relation of §4 and by nothing else.
...
*Figure 3. Types of the registered operators (`Prim.ty`), abridged. Dimension algebra lives here and nowhere else. $\mathsf{eq}$ is available at every data type and $\mathsf{lt}$ at quantities only.*
...
*The construction boundary.* Client code is typed under $\varnothing$; a declaration's realization is typed under $\mathrm{grant}$ its own expected type (§4.1). A value of $C$ is therefore constructed only inside a declaration whose signature announces $C$: the signature is the realization's authority, and §5 shows what each weaker alternative admits.

*The temporal boundary.* $\mathsf{delay}$ and $\mathsf{sync}$ are typed only in the empty context and only at data types. Both restrictions were forced by the totality proof of §6, not chosen: a delayed closure would have to be transported across ticks, and a delay under a binder would re-evaluate its operand at the previous tick in an environment created at the current one. Temporal state therefore belongs to declarations — memory is a property of a relationship, not of a function — and relationships with inputs are pointwise — the arrangement of `pre` in Lustre, where it lives in nodes rather than in functions [@halbwachs1991lustre].
...
What discharges a commitment is not the kernel's business; it is the validation layer's. The kernel abstracts it as an **evidence** relation $\mathit{ev} : \mathsf{DeclEnv} \to \mathsf{Expr} \to \mathsf{PropertyId} \to \mathsf{Prop}$. Evidence takes the environment because compositional discharge needs it — "$A$ is monotone because $B$ is committed to be monotone" consults $B$'s interface. A realization $e$ **satisfies** $\mathcal{P}$ in $\Theta,\Delta,\Gamma$ when it has the expected type under the grant of that type and every commitment is discharged:
$$
...
Separately from the steps there is a purely structural order with no satisfaction condition: $\mathsf{DeclLeq}\;h\;h'$ requires the same identity, $h.\mathcal{P} \sqsubseteq h'.\mathcal{P}$, and a write-once realization ($h.\mathit{realization} = \mathsf{some}\;e \Rightarrow h'.\mathit{realization} = \mathsf{some}\;e$); $\mathsf{EnvRefines}\;\Delta\;\Delta'$ lifts it pointwise and permits new declarations. Storing a refined declaration back under its identity is an environment refinement — $\Delta\;h.\mathit{id} = \mathsf{some}\;h \wedge \mathsf{DeclLeq}\;h\;h' \Rightarrow \mathsf{EnvRefines}\;\Delta\;(\Delta[h'])$ (`EnvRefines_update`) — and this is the one place identity does any work: it makes the update land on the slot every reference resolves to, which is what a name does in any environment semantics.
...
**Theorem 3 (Clients survive realization — typing; `local_refinement_preserves_global_typing`).** If $\Delta\;B = \mathsf{some}\;h$ and $\mathsf{DeclLeq}\;h\;h'$, then every judgment $\Theta;\Delta;\Gamma \vdash_{G} e : \tau$ holds in $\Delta[h']$.
...
**Definition (Monotone evidence).** $\mathit{ev}$ is **monotone** when $\mathsf{EnvRefines}\;\Delta_1\;\Delta_2 \wedge \mathit{ev}\;\Delta_1\;e\;p \Rightarrow \mathit{ev}\;\Delta_2\;e\;p$. Evidence that ignores the environment is monotone; evidence that consults only the *presence* of commitments and realizations is monotone; evidence that consults their *absence* is not.

**Theorem 4 (Clients survive realization — commitments; `local_refinement_preserves_global_wf`, `local_lifecycle_preserves_global_wf`).** If $\mathit{ev}$ is monotone, $\mathsf{GlobalWF}\;\mathit{ev}\;\Theta\;\Delta$, $\Delta\;B = \mathsf{some}\;h$ and $h \rightsquigarrow h'$ with side conditions checked in $\Delta$, then $\mathsf{GlobalWF}\;\mathit{ev}\;\Theta\;(\Delta[h'])$. The same holds for a whole lifecycle $h \rightsquigarrow^{*} h'$ checked against the original $\Delta$.
...
Let $\eta : \mathsf{ConceptId} \to \mathsf{Ty}$ map each concept to a data type, agreeing with $\Theta$ on bound concepts. Erasure $\tau^{\eta}$ replaces $C$ by $\eta\;C$ throughout a type; on terms, $\mathsf{rep}\;e$ and $\mathsf{mk}_C\;e$ erase to $e^{\eta}$, and the type indices of operators are erased.
...
*Figure 4. Single-domain evaluation (`Ev`), with $\Delta$ and $I$ ambient. In one domain $\mathsf{sync}_{\kappa}$ evaluates exactly as $\mathsf{delay}$ (rules `syncZero`, `syncSucc`), which §6.7 justifies. Literals evaluate to themselves. $\#i$ is de Bruijn index $i$.*
...
Let $e.\mathsf{instRefs}$ be the declarations $e$ refers to *instantaneously*: those not under the delayed operand of a $\mathsf{delay}$ or $\mathsf{sync}$ (the initial value is read at tick $0$ and counts as instantaneous). $\mathsf{InstDependsOn}\;\Delta\;a\;b$ holds when $b \in \mathsf{instRefs}$ of $a$'s body.

**Definition (Causal).** $\mathsf{Causal}\;\Delta := \exists\,\mathit{rank}\,R.\;(\forall \delta.\;\mathit{rank}\;\delta < R) \wedge \forall a\,b.\;\mathsf{InstDependsOn}\;\Delta\;a\;b \to \mathit{rank}\;b < \mathit{rank}\;a$.
...
**Proposition 10 (Strict cycles have no value; `Ev.not_of_strictCyclic`).** If $a$ lies on a cycle of references passing through neither a delayed operand nor a lambda, then for every tick and environment there is no $v$ with $\rho \vdash_t \mathsf{declRef}\;a \Downarrow v$.
...
Not "some default", not "one of several": no derivation exists. A gap should be recorded. A cycle guarded by a lambda, `A := λx. A x`, is rejected by $\mathsf{Causal}$ yet `declRef A` does evaluate — to a closure; only applying it diverges. $\mathsf{Causal}$ is conservative for lambda-guarded cycles and Proposition 10 covers strict cycles only.
...
*Proof sketch.* Lexicographic induction on $(t, r, \mathsf{derivation})$. A delayed operand at tick $t+1$ is evaluated at tick $t$ under *any* rank (the first component decreases); an instantaneous reference to a realized declaration $\delta$ is evaluated at the same tick under the smaller bound $\mathit{rank}\;\delta$ (the second decreases), and its realization is well typed under the grant of its own signature by $\mathsf{GlobalWF}$; every other case is the induction on the derivation. The $\mathsf{fold}$ case uses `fold_total` (§7.1). $\square$
...
$$
$\mathsf{delay}$ reads the previous activation of the current domain; $\mathsf{sync}_{\kappa'}$ reads the previous activation of $\kappa'$ and evaluates its operand *there*, in $\kappa'$. All other rules carry $\kappa$ unchanged.
