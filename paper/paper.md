...
The declaration model above is the frame every later result is stated in. Semantic identity, representation binding and dimensions are Part IV; the data core, the equation library and the natural surface Part V; units and charts Part VI; the reactive semantics, clock domains and buffering Part VII; behavior components and groups Part VIII; the environment boundaries — the canonical type `() -> B`, the source role, logical outputs, provision and realization — Part IX; hardware and capacity validation Part X. The minimality verdict for each construct is given where the construct is discussed and gathered in Part XIV.
...
| FVD-0137 | Hardware requirements are a validation judgment separate from the encoder | superseded by FVD-0139 | Part IX | Phase 14 | ADR-0015 (supports) |
...
| FVD-0139 | Deployment admissibility is the encoder's typing, its fit and a solvable board; the narrow fit-and-allocate predicate is not admissibility | accepted (supersedes FVD-0137) | Part IX | Phase 14 hardening | ADR-0015 (supports) |
