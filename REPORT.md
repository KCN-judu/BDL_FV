...
Project state: **Phase 4 complete** (reactive core).  Remaining, on the
consolidated roadmap: Phase 5 clock domains + synchronization; Phase 6
actions / effects / arbitration; Phase 7 validation + invalidation; Phase 8
surface elaboration + executable semantics; final minimality audit.  Everything builds
...
delay init e    the value of e one tick ago, init at tick 0                 (Phase 4: the only state primitive; data-typed, top level)
...
semantics       Ev Δ I t ρ e v: tick-indexed evaluation; unresolved declarations are inputs; Ty unchanged
                (no Signal, no Event); executable iff Causal (instantaneous graph acyclic)
...
| `BDL/Core/Reactive.lean` | Phase 4: `Value`, `Ev`, `Ev.det`, `evalF`+soundness, strict cycles, logical relation, `fundamental`/`reactive_total`, `Ev.tag_provenance`, `unfolds_preserves_eval` |
...
* Event multiplicity, merge policies — Phase 5/6.
