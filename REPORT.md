...
Project state: **Phase 10 complete** (unit coordinates: `inUnit`/`withUnit`
as elaborated quantity arithmetic, exact symbolic unit semantics with π,
Formula Composer local dimension inference proved sound and complete,
presentation invariance, affine units: conversion exact, arithmetic sort
deferred; on top of 9a–9c).  Remaining: Phase 8c surface elaboration of the remaining
...
* ~~Representation binding for concepts~~ — resolved in Phase 3 (§3.3); the
  ...
  mechanism.
* Affine units: conversion and display elaborate exactly (Phase 10); the point/difference sort for °C arithmetic is deferred.  The concept display-name table — surface, not
  modelled.
