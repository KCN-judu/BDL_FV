...
theorem step₀₁ : DeclRefines ev .empty [] h₀ h₁ := .refine (by decide)
theorem step₁₂ : DeclRefines ev .empty [] h₁ h₂ := .refine (by decide)
theorem step₂₃ : DeclRefines ev .empty [] h₂ h₃ := .realize (by decide)
theorem lifecycle : DeclRefinesStar ev .empty [] h₀ h₃ :=
  .step step₀₁ (.step step₁₂ (.single step₂₃))

example : h₀.id = h₃.id := lifecycle.id_eq
example : Satisfies ev .empty [] idNat S₀ :=
  lifecycle.final_realization_satisfies_all (WellFormedDecl.unresolved ev _ [] dId S₀) rfl
...
inductive NaiveDeclRefines (ev : Evidence) (Δ : DeclEnv) (Γ : Ctx) : DesignDecl → DesignDecl → Prop where
  | refine {id S S'} (h : InterfaceRefines S S') :
      NaiveDeclRefines ev Δ Γ ⟨id, S, none⟩ ⟨id, S', none⟩
  | realize {id S e} (hs : Satisfies ev Δ Γ e S) :
      NaiveDeclRefines ev Δ Γ ⟨id, S, none⟩ ⟨id, S, some e⟩
  | strengthen {id S S' e} (h : InterfaceRefines S S') :
      NaiveDeclRefines ev Δ Γ ⟨id, S, some e⟩ ⟨id, S', some e⟩

theorem naive_breaks_wellformedness :
    ¬ ∀ (h₁ h₂ : DesignDecl),
        WellFormedDecl ev .empty [] h₁ → NaiveDeclRefines ev .empty [] h₁ h₂ →
        WellFormedDecl ev .empty [] h₂ := by
  intro h
  have wf₁ : WellFormedDecl ev .empty [] ⟨dId, S₁, some constZero⟩ := by decide
  have step : NaiveDeclRefines ev .empty [] ⟨dId, S₁, some constZero⟩ ⟨dId, S₂, some constZero⟩ :=
...
theorem probe1_step : DeclRefines ev Δ₀ [] hB₀ hB₁ := .refine (by decide)
...
theorem probe2_step : DeclRefines ev Δ₀ [] hB₁ hB₂ := .realize (by decide)

theorem probe2_lifecycle : DeclRefinesStar ev Δ₀ [] hB₀ hB₂ := .of_two probe1_step probe2_step
...
theorem probe5_breaks_commitment : ¬ WellFormedDecl compEv Δ₅' [] dA_mon := by decide
...
theorem probe6_step : DeclRefines badEv Δ₆ [] hB₀ hB₆ := .realize (by decide)
