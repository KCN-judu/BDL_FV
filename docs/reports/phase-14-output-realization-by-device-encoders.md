...
Question (the output-side dual of Phase 13; production ISS-0016's other half
and the `output adapter` of `docs/spec/runtime-semantics.md`): how can a
...
logical output of Phase 6 already is that boundary; realization is a
_lowering_ that adds one pure encoder declaration and one machine sink and
changes nothing the behaviour can observe; the machine boundary is a relation
on commands, not a term. Files: `BDL/Surface/OutputRealization.lean`,
...
- `Encoder = ⟨rep, raw, encode, transfer, rep_semFree, rep_data, raw_semFree, raw_data, encode_pure, computes⟩`
  ...
  transfer function on values, and `computes : ∀ v, TyVal rep v → Transduces encode v (transfer v)`.
  Both `rep` and `raw` are sem-free data: an encoder observes a representation
  and produces a command; it never handles a concept value.
...
- **Model B — the specification, on the unchanged design.**
  `RawCommand S Δ I Ω β R t w := ∃ spec v, Ω o = some spec ∧ PhysicalOutput … o t v ∧ w = transfer (unwrapAt spec.accepts v)`.
  This is the machine boundary: what sink `p` receives at tick `t`. No BDL
  term, no evaluation rule, no effect.
- **Model C — the lowering.** `lowerΔ Δ R spec = Δ[e ↦ ⟨raw, []⟩ realized by encoderBody]`
  with `encoderBody (sem c) = app encode (rep (declRef d))` and
...
- `DeviceOutputProfile = ⟨E, requirements⟩` and
  `Admissible Θ accepts H P := EFits ∧ (solve H P.requirements).isSome` — the
  encoder and the hardware requirements are two judgments.
...
| **platform independence**: two realizations of one design evaluate every pre-existing term alike                                              | `two_realizations_same_behavior`                                                                               | `NoMention` and avoiding inputs for both                                                                 |
...
| validation: fit plus a solvable board gives a valid assignment                                                                                | `admissible_satisfiable`                                                                                       | `Admissible`                                                                                             |
...
profile not admissible for the relay), `exI`/`exI_theorem` (Model A and the
implicit clock crossing).

Seventy theorem-like declarations (48 + 22); every one on
...
- _definitional_: `behavior_unchanged` (an `update` off its key);
  `RawCommand` as the machine boundary; `Admissible` as a conjunction.
- _not established_: codegen refinement (the generated `Outputs` struct and the
  future adapter call against `RawCommand`); stateful output adapters
  (slew-rate limiting, dithering, batching, hysteresis); a device clock
  different from the output clock; atomic multi-value frames; commitments on
  outputs (production authors none) — FVI-0022.
...
| hardware requirements of a device                                        | MOVE TO VALIDATION (`Admissible`)                                           | FVD-0137 |
| injectivity / round-trip of the encoding                                 | REMOVE — not required; quantization admitted                                | FVD-0135 |
| many-to-one output lowering                                              | DEFER — singleton primitive; combine upstream or batch per tick             | FVD-0136 |
