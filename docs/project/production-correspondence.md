...
area: process
status: current
snapshot: aa6e7f4ee249556ca7561c854b43607f45f229fe
...
commit `aa6e7f4ee249556ca7561c854b43607f45f229fe` (2026-09-20, HEAD of `main`
after the Arduino Nano target over `avr-hal` — the second adapter family, the
vocabulary crate renamed `bdl-runtime-adapter` (ADR-0037 amended) —, the
monograph's reference mirror under `reference/paper/`, and six Studio commits
(one pointer state machine, CAD selection, the type scale) that touch no
boundary this page records; protocol 0.24)**; the `snapshot` field above is the
one canonical place that hash lives, and the monograph's production snapshot
cites it. The pin moves when a row does. It does not grade production's labels.
The direction of authority is fixed by production's ADR-0010: the Lean
development is a specification, never a dependency.
...
| ISS-0016 (open)     | 13, 16  | FVD-0121 … FVD-0130, FVD-0144 … FVD-0146; FVI-0012, FVI-0020, FVI-0029          | a device binding for a Source: the formal construction exists (PRP-0001 audit); Phase 16 adds what the binding is (`assignSource` = the provision with a catalogue entry, FVD-0146), that two providers with one semantic trace are indistinguishable (`two_providers_same_behavior`, FVD-0144) and what the binding must promise about occurrences (FVI-0029); nothing implemented — the first adapter refuses a design with a Source (`adapter.inputs_unbound`); the Source sheet (protocol 0.23) creates a Source over a chosen concept and is authoring, not a device binding | —                                                                                                                             |
| ISS-0017 (open)     | 14–16   | FVI-0023 … FVI-0027 (FVI-0022 split); FVD-0145 … FVD-0148                       | output realization beyond a pure encoder: stateful adapters, a device clock, atomic frames, the adapter's correspondence, a device catalogue — production's mirror of FVI-0022; Phase 16 answers the catalogue half in the model (a profile is a value satisfying the existing contract, its origin unread, `assign` is the lowering, contract separate from feasibility) and the paired-axis case (one output, a pair command; no transaction primitive, FVD-0148)                                                                                                               | —                                                                                                                             |
